...
multi::multi(group* parent, const char* title, const std::vector<std::pair<const char*, bool*>> list)
{
	this->title		= title;
	this->list		= list;
	this->distance	= { 0, 21 };
...
void multi::draw()
{
	...
	if (events->has_focus(this))
	{
		for (int i = 0; i < this->list.size(); i++)
		{
			...
			// selected -> black.
			if (*this->list[i].second || in_bound)
			{
				// list items text.
				fonts->segoe_ui.text(list_area.x + 8, list_area.y + (list_area.h / 2) - 8, this->list[i].first, color(255, 255, 255));
			}
			// not selected -> grey.
			else
			{
				// list items text.
				fonts->segoe_ui.text(list_area.x + 8, list_area.y + (list_area.h / 2) - 8, this->list[i].first, color(120, 120, 120));
			}
		}
	}
...
void multi::think()
{
	...
	if (events->has_focus(this))
	{
		for (int i = 0; i < this->list.size(); i++)
		{
			...
			// selecting items in list area.
			if (input->in_bound(list_area) && input->key_pressed(VK_LBUTTON))
				*this->list[i].second = !*this->list[i].second;
		}
	}
}
