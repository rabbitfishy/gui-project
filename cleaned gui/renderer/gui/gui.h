...
namespace gui
{
	...
	{
	public:
		multi(group* parent, const char* title, const std::vector<std::pair<const char*, bool*>> list);

		void draw()					override;
		void think()				override;

	private:
		std::vector<std::pair<const char*, bool*>> list;

		std::string construct_list(const std::vector<std::pair<const char*, bool*>> list)
		{
			...
			for (int i = 0; i < list.size(); i++)
			{
				...
				if (*list[i].second && !length_exceeded)
				{
